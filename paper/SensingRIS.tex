...
\subsection{Simultaneous Rotational Signaling and Interference Detection} \label{Simultaneous Rotational Signaling and Interference Detection}
    ...
    However, according to \eqref{traditional beamforming}, it is the phase sum of the RIS-user channel and the BS-RIS channel, i.e., $\arg({\bm g}_n^{T}{\bm w})+\arg(f_n^*w')$, that determines the optimal phase-shift of the $n$-th element. Given the phase difference $\varphi = \arg\left(f_{n}^{*}w'\right)-\arg\left(\bm g_{n}^{T}\bm w\right)$, in order to acquire the phase sum, we only need to obtain one of the phases of these two channels to perform optimal RIS beamforming. 
    Though generally it is difficult to obtain the BS-RIS channel and the RIS-user channel, since the BS and RIS are installed at fixed locations, it is reasonable to assume that the BS-RIS channel is much more stable and predictable, compared to the RIS-user counterpart. 
    Specifically, in a common mmWave scenario, the NLOS paths are often highly attenuated. 
    Thus, the channel is mainly composed of only one LOS path, which is fully determined by its geometry \cite{bai2014coverage}, i.e., the locations of the BS and the RIS. 
    Different from the fast time-varying nature of the user-RIS link, the BS-RIS channel often exhibits a quasi-static property \cite{Huchen}. 
    This property significantly simplifies the channel estimation of the BS-RIS channel.
    As a result, the phases of the BS-RIS link $\arg({\bm g}_n^{T}{\bm w})$ can be assumed to be known in our algorithms, and are calculated from the BS-RIS geometry in our simulations. 
...
\subsection{Properties of the CRLB} \label{Properties of the CRLB}
    From the above analysis, we have obtained the exact and asymptotic CRLB expressions, where we can observe two properties of the phase estimation problem:
    \begin{enumerate}
...
\subsection{Achievable Spectral Efficiency under IRF} \label{Achievable Spectral Efficiency under IRF}
    ...
    In the simulation of our IRF method, the VM-EM phase estimation algorithm is utilized with the assumption that the phases of the BS-RIS LOS path are known. 
