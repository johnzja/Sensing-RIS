...
Secondly, since the number of RIS elements is usually large, the dimensions of the channels increase sharply compared with conventional systems, which results in unacceptably high pilot overhead for channel estimation.
...
Another architecture utilizes only one \ac{RF} chain, which also aims to have an estimate on the array gains of RIS \cite{alexandropoulos2020hardware}. % Add evaluations.
...
\subsection{Properties of the CRLB}
    ...
    \begin{figure}[!t]
        ...
        \caption{Precise $1/{\rm CRLB}(\varphi)$ as a two-variable function of $K$ and $\bar{\gamma}$, calculated from \eqref{eqn:precise_CRLB}. The larger the value of reciprocal CRLB, the more precise the estimator can be. }
