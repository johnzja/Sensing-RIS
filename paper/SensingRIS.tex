...
\label{Models for IRFs}
    ...
    \begin{equation}
        E(t) = \sqrt{2} {\rm Re}\left( E_{{\rm BB}}(t)e^{{\rm j}\omega_c t} \right),
...
\begin{tabular}{|l|r|r|r|r|}
            ...
            \hline 
            Two-timescale\cite{Huchen} & $\frac{2(N+1)}{\alpha} + K\lceil N/M\rceil +K$ &RIS& Slow variation  \\
            \hline 
            Proposed IRF & 3 & Sensing RIS  &General\\ 
            \hline
    \end{tabular}
\end{table}
...
\label{Performance Analysis}
    ...
    Preliminaries about the \ac{CRLB} will be introduced in Subsection~\ref{The CRLB}, and then the exact and asymptotic expressions of the CRLB will be derived in Subsection~\ref{Precise CRLB} and Subsection~\ref{Asymptotic CRLB}, respectively. Finally, the mathematical properties of the obtained \ac{CRLB} and error analysis will be provided in Subsection~\ref{Properties of the CRLB}. 
...
\subsection{Asymptotic CRLB}    \label{Asymptotic CRLB}
    ...
    \end{IEEEproof}
    \begin{remark}
        The asymptotic result of the CRLB still relies only on two parameters $K$ and $\bar{\gamma}$. But the calculation is much simpler compared to the exact expression of the CRLB. 
    \end{remark}

\subsection{Properties of the CRLB} \label{Properties of the CRLB}
    From the above analysis, we have obtained the exact and asymptotic CRLB expressions. Furthermore, from these expressions, we can observe two properties of the phase estimation problem:
    \begin{enumerate}
...
\section{Proof of \textbf{Theorem 1}}\label{appendix:proof of theorem1}
    According to the definition of the CRLB \eqref{eqn:CRLB definition}, taking the negative expectation of \eqref{Second Derivative Likelihood} yields the reciprocal CRLB of the estimators for $\varphi$. Note that in \eqref{Second Derivative Likelihood}, there are three types of expectations to be evaluated: $\mathbb{E}\left[P[l]\right]$, $\mathbb{E}\left[R(z_l)\sqrt{P[l]}\right]$, and $\mathbb{E}\left[(1-R^2(z_l))P[l]\right]$. The expectation of $P[l]$ can be directly evaluated from \eqref{eqn:sensor power} by the linearity of the expectation operation:
    \begin{equation}
