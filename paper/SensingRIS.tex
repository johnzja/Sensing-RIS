...
Firstly, in contrast to traditional antenna array capable of transmitting, receiving, and processing the signals, RIS can only passively reflect the incident signals.
...
These two main challenges result in the urge need of new channel estimation techniques for beamforming in RIS-assisted communications.
...
To further reduce the number of active sensors, in \cite{alexandropoulos2020hardware} the authors proposed an alternating direction method-based channel estimation procedure with a single \ac{RF} chain by the help of an extended analog combiner \cite{vlachos2019wideband}. Since RF chains have been attached to RIS for channel sensing in these works, they can also be modified to perform signal relay. In \cite{nguyen2021hybrid}, a hybrid relay-reflecting architecture is considered, where a few passive RIS elements are replaced by the active amplify-and-forward relay modules. These active modules are then capable of channel estimation. 

The second category preserves the original hardware architecture of RIS, while conceives new algorithms to exploit the specific features that RIS brings about.
...
To circumvent the problem of high pilot overhead required for channel estimation in RIS-assisted systems, we point out that the main drawback of conventional channel estimation methods is that they only extracts the mathematical features of channels, while neglects their electromagnetic nature. Thus, we propose an \ac{IRF}-based approach, where the \ac{IRF} on RIS is utilized for channel estimation\footnote{Simulation codes will be provided to reproduce the results in this paper: \url{http://oa.ee.tsinghua.edu.cn/dailinglong/publications/publications.html}.}.
Specifically, the contributions of this paper can be summarized as follows.
    \begin{itemize}
        ...
        Inspired by optical interference where phase information can be obtained from the interference fringes, we reveal that phase information about the channel can also be obtained from the phenomenon of electromagnetic interference that occurs on RIS, which we name as the \ac{IRF}. 
        Fortunately, phase information is enough for RIS beamforming that serves a single-antenna user. 
        ...
        \item
        To exploit this \ac{IRF} for CSI acquisition, we employ sensing RIS which integrates power detectors into the RIS elements to capture the features of the \ac{IRF}. Each of the power detectors can acquire its own phase information independently from the \ac{IRF}, so that the required pilot overhead is independent of the number of RIS elements. 
        We then develop LS, ML and VM-EM phase estimation algorithms to extract the phase information from the power detector signals in order to perform beamforming. 
        \item
...
\label{Interference Random Field}
    In this section, the signal model for \ac{IRF} will be first introduced in Subsection~\ref{Models for IRFs}. Then, in order to utilize the \ac{IRF} for low-overhead CSI acquisition, power sensors will be attached to the RIS elements in Subsection~\ref{probe the channel directly on the RIS}. Finally, to make the power signal vary in a desired form that is rich in CSI, simultaneous rotational signaling method will be proposed in Subsection~\ref{Simultaneous Rotational Signaling and Interference Detection}. 

    Interference is a fundamental physical phenomenon of all kinds of waves. The most well-known example is the double-slit optical interference \cite{louradour1993interference}, as is shown in Fig.~\ref{fig:scheme}~(a), where the interference fringes created during the wave superposition reveal the phase difference of the two optical paths. Similarly, the same interference phenomenon occurs when two RF electromagnetic waves meet together on the RIS, as is shown in Fig.~\ref{fig:scheme}~(b), and the \ac{IRF} created by this interference reveals the \ac{CSI}. 
    \begin{figure}[t]
        \centering
        \subfigure[Double-slit optical interference.]
        {\includegraphics[width=.6\textwidth]{figures/scheme1.pdf}}
        %\vspace{0.2mm}
        \subfigure[\ac{IRF} on RIS, created by transmitting signals from the \ac{BS} and the user to RIS simultaneously.]
        {\includegraphics[width=.6\textwidth]{figures/scheme2.pdf}}
        \caption{Analogy between the optical interference and the \ac{IRF} phenomenon occured on RIS.}
        \label{fig:scheme}
        %\vspace{-0.5em}
    \end{figure}
...
\label{Models for IRFs}
    Suppose two signals imping the RIS simultaneously, creating an interference field at the $n$-th RIS element, with both of the symbols transmitted by \ac{BS} and \ac{UE} being $s=1$. In order to perform channel estimation, we need to probe the interference fringes, i.e., the power of the \ac{IRF}. To clarify the power problem associated with an electromagnetic signal, the relationship between physical signal power and baseband equivalent signal power is re-stated here. Both of the BS-RIS and user-RIS signals that appear on each RIS element can be represented by their baseband equivalent signals $E_{n,{\rm BB}}(t)$, 
    ...
    $\lVert E_{n, {\rm BB}}\rVert_2^2 = \lVert E_{n}\rVert_2^2$. Thus, we are sure that taking the square of the baseband signal is equivalent to calculating the power of the physical electromagnetic signal. 
    Now we consider the superposition of the BS-RIS signal and the user-RIS signal. Due to the linearity of \eqref{baseband-passband relationship}, we can simply add up the two impinging baseband signals to represent the interference signal that appears at the RIS, i.e., 
    \begin{equation}
        E_{n,{\rm BB, IRF}}(t) = E_{n, {\rm BB, BS}}(t)+E_{n,{\rm BB, UE}}(t),
    \end{equation}
    where 
    \begin{equation}
        E_{n, {\rm BB, BS}}(t) = \bm g_{n}^{T}\bm w,
    \end{equation}
    and 
    \begin{equation}
        E_{n,{\rm BB, UE}}(t) = f_{n}^{*} e^{{\rm j}\psi(t)}.
    \end{equation}
    Thus, the noisy \ac{IRF} signal is
    \begin{equation}
        \label{interference}
        E_{n,{\rm BB, IRF}}(t)=\bm g_{n}^{T}\bm w+f_{n}^{*}e^{{\rm j}\psi (t)}+v(t),
    \end{equation}
    where $v(t)$ is the electromagnetic noise signal in its baseband representation, and $\psi(t)$ is the time-dependent phase difference between the symbols transmitted by \ac{BS} and the user.  
    Furthermore, by defining $\alpha = \left\vert\bm g_{n}^{T}\bm w\right\vert$, $\beta = \left\vert f_{n}^{*}\right\vert$, and the phase difference between the \ac{BS}-\ac{RIS} channel and the \ac{RIS}-user channel as $\varphi = \arg\left(f_{n}^{*}\right)-\arg\left(\bm g_{n}^{T}\bm w\right)$, then the detected power of the \ac{IRF} can be written as
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    where for every time instant $t\in {\mathbb R}$, the electromagnetic noise $v'(t)\overset{\text{d}}{=}v\sim\mathcal{CN}\left(0,\sigma_{v}^{2}\right)$, $\zeta$ is the noise introduced by digital signal processing after measuring the field power, and $A$ is the amplification factor of the power sensor. 
    ...
    The phase difference $\varphi$ defined here will play an important role in our following channel estimation, since it carries enough CSI for beamforming. To estimate the phase difference $\varphi$, we need enough observations from the sensor-detected power signal $P(t)$. For simplicity and without loss of generality, we can assume $\psi(t)=\omega t$ and $\omega=\frac{2\pi}{T_{s}}$, where $T_s$ is the symbol period. Furthermore, assume that $L$ observations $P[l]=P(t_{l})$ located at equally-spaced instants 
    \begin{equation}
...
\subsection{Probe the Channel Directly on the RIS}  \label{probe the channel directly on the RIS}
    Most of the existing channel estimation methods use the transceivers to obtain  CSI. 
    In these methods, specially-designed electromagnetic waves are emitted by the transmitter, which imping the RIS, and the bounced-back waves enter the receiver, carrying information about the channel. 
    In a word, these methods all use electromagnetic waves to probe the radio environment. 
...
\subsection{Simultaneous Rotational Signaling and Interference Detection} \label{Simultaneous Rotational Signaling and Interference Detection}
    ...
    %     \label{fig:scheme}
    % \end{figure}
    % If we allow the BS and UE to emit EM waves simultaneously, then the interference phenomenon will occur (see Fig.~\ref{fig:scheme}), and interference fringes \cite{louradour1993interference} will appear on the RIS. Then, if we alter the phase at the UE, the interference fringes will change accordingly, allowing the power sensors on the RIS to obtain CSI. 
    \begin{figure}[!t]
        \centering
    ...
    As is introduced in previous subsections, if we allow the BS and \ac{UE} to transmit electromagnetic waves simultaneously, then the interference phenomenon will occur on each RIS element. 
    However, equal carrier frequencies of the BS and \ac{UE} create stable interference fringes \cite{louradour1993interference} on the RIS, which do not carry information about the channel. 
    ...
    The phase of this sinusoidal waveform is uniquely determined by $\varphi$.
...
\label{Performance Analysis}
    ...
    Preliminaries about the evaluation of \ac{CRLB} will be introduced in Subsection~\ref{The CRLB}, and then precise CRLB and asymptotic CRLB will be derived in Subsection~\ref{Precise CRLB} and Subsection~\ref{Asymptotic CRLB}, respectively. Finally, the mathematical properties of the obtained \ac{CRLB} and error analysis will be provided in Subsection~\ref{Properties of the CRLB}. 
...
\label{Conclusion}
    ...
    Furthermore, when equipped with a sensing RIS, traditional MIMO systems can also benifit from the additional CSI provided by the phase estimation methods of IRF.  
...
\section{Proof of \textbf{Theorem 1}}\label{appendix:proof of theorem1}
    ...
    %The introduction of operation $(x)^{+}$ is due to the non-negativity of each of the $L$ CRLB components. 
...
\label{Proof of Theorem 3}
    ...
    \end{figure}
    then, from \eqref{eqn:approx evaluation}, we have 
    \begin{equation}
    ...
    \end{equation}
    Thus, $1/\gamma - \hat{g}(\gamma) \sim 1/2\,\Theta(1/\gamma)$, and the relative error $r$ of a single term in the CRLB expression is upper bounded by introducing a positive parameter $\epsilon$:
    \begin{equation}
        \begin{aligned}
