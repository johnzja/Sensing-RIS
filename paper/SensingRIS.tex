...
Equivalently, \eqref{CE received signal} can be expressed as the matrix form
    ...
    \label{eqn:LS_CE model}
        {\bm Y}_{\rm BS}=\bm H^{T} \bm F_{N,P}x + {\bm N},
    \end{equation}
...
\label{Models for IRFs}
    ...
    Now we consider the superposition of the BS-RIS signal and the user-RIS signal. Due to the linearity of \eqref{baseband-passband relationship}, we obtain the noisy IRF signal by adding up the two impinging baseband signals as
    \begin{equation}
    \begin{aligned}
    E_{n,{\rm BB, IRF}}(t) &= E_{n, {\rm BB, BS}}(t)+E_{n,{\rm BB, UE}}(t)+v(t)\\
    &=\bm g_{n}^{T}\bm w+f_{n}^{*}e^{{\rm j}\psi (t)}+v(t),
    \end{aligned}
    \end{equation}
    ...
    Furthermore, by defining $\alpha = \left\vert\bm g_{n}^{T}\bm w\right\vert$, $\beta = \left\vert f_{n}^{*}\right\vert$, and the phase difference between the \ac{BS}-RIS channel and the RIS-user channel as $\varphi = \arg\left(f_{n}^{*}\right)-\arg\left(\bm g_{n}^{T}\bm w\right)$, then the detected power of the \ac{IRF} can be written as
    \begin{equation}
...
\subsection{Simultaneous Rotational Signaling and Interference Detection} \label{Simultaneous Rotational Signaling and Interference Detection}
    ...
    % If we allow the BS and UE to emit EM waves simultaneously, then the interference phenomenon will occur (see Fig.~\ref{fig:scheme}), and interference fringes \cite{louradour1993interference} will appear on the RIS. Then, if we alter the phase at the UE, the interference fringes will change accordingly, allowing the power sensors on the RIS to obtain CSI.    
        ...
        \caption{Simultaneous rotational signaling scheme. The IRF power is the instantaneous total power of the BS-RIS signal and the user-RIS signal. The composite power waveform detected by the power sensor enables our algorithm to obtain the desired CSI. The parameters $\alpha$ and $\beta$ are estimated from the first 2 time slots, and $\varphi$ is estimated by IRF that occurs during the third time slot.}
        \label{fig:protocol}
    \end{figure}

    To clarify this idea, we take only one RIS element into consideration. 
    As is introduced in previous subsections, if we allow the BS and the user to transmit electromagnetic waves simultaneously, then the interference phenomenon will occur on each RIS element. 
