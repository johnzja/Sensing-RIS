...
\label{Models for IRFs}
    ...
    \else 
        ...
        \end{equation}
    \fi
    where $\zeta$ is the noise introduced by digital signal processing after measuring the IRF power, $A$ is the amplification factor of the power sensor, and the equivalent electromagnetic noise $v'(t) = e^{\ri{\rm arg}({\bm g}_n\T \bm w)}v^*(t)$ follows the same distribution as $v(t)$ for any time $t$.
...
\red{\begin{theorem}[MSE of the vM-EM estimator] \label{thm:asymp_perf_vM-EM}
...
\end{equation} 
If we apply the constraint $\|{\bm v}\|/\sqrt{L}\leq \delta$, then $t$ can be chosen to $t=1$, i.e., 
\begin{equation}
