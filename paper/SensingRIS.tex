...
\label{Channel Estimation}
    ...
    Given the received signal $\bm Y_{\rm BS}$ and, without loss of generality, assuming $w's'=\sqrt{P'_{\rm max}}$, the channel estimation problem in RIS-assisted system can be obtained by the MMSE estimator~\cite{kundu2021channel} as  
    \begin{equation}
        \hat{\bm H}\T = \mathbb{E}\left[ {\bm H}\T | {\bm Y}_{\rm BS} \right]. 
    \end{equation}
    ...
    An alternative way to circumvent this difficulty is to approximate the MMSE estimator with the best linear estimator. By assuming $\sigma_h^2 = \sigma_f^2\sigma_g^2$, an LMMSE estimator can be obtained as 
    \red{
    \ifx\onecol\undefined
        \begin{equation}
            \begin{aligned}
            & (\hat{\bm{H}\T})_{\rm LMMSE} \\
    ...
    \else
        \begin{equation}
            \hat{\bm{H}\T}_{\rm LMMSE} = \sqrt{P'_{\rm max}} {\bm Y}_{\rm BS} {\bm F}_{N, N_p}\H \left( P'_{\rm max}{\bm F}_{N, N_p} {\bm F}_{N, N_p}\H + \frac{\sigma_z^2}{\sigma_h^2}{\bm I}_{N}  \right)^{-1},
            \label{eqn:LMMSE}
        \end{equation}
    \fi
    }
    which provides a feasible solution to the channel estimation problem in the RIS-assisted MISO system. In this paper, we name it the linear minimum mean squared error for $\bm H$ (LMMSE-$\bm H$) channel estimation method. 
    ...
    \end{equation} 
    which immediately leads to the following MMSE\footnote{This is the true MMSE estimator, since the model is linear, and the prior on $\bm f$ and the noise $\bm n$ is Gaussian. } estimator for $\bm f$:
    \begin{equation}
    ...
    \begin{equation}
        {\bm A} = \left[ {\bm G}\T\diag({\bm \theta}_1);\cdots;  {\bm G}\T\diag({\bm \theta}_{N_p})\right] \in \mathbb{C}^{N_pM\times N}.
    \end{equation}
