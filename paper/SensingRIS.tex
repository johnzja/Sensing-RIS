...
\label{Models for IRFs}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    where for every time instant $t\in {\mathbb R}$, $\zeta$ is the noise introduced by digital signal processing after measuring the IRF power, $A$ is the amplification factor of the power sensor, and the equivalent electromagnetic noise $v'(t) = e^{{\rm j}{\rm arg}({\bm g}_n^T \bm w)}v^*(t)$ follows the same distribution as $v(t)$.
