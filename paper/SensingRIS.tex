...
These two main challenges result in the urge need of new channel estimation techniques for RIS-assisted communications.
...
For example, the authors of \cite{taha2021enabling} have proposed an architecture where several RF chains are connected to part of the RIS elements, and baseband controllers are employed to process the received pilot signals for channel estimation.
Another architecture utilizes only one \ac{RF} chain, which also aims to have an estimate on the array gains of RIS \cite{alexandropoulos2020hardware}. % Add evaluations and comments for these methods. 
In \cite{nguyen2021hybrid}, a hybrid relay-reflecting architecture is considered, where a few passive RIS elements are replaced by the active relay devices.
These active elements are then capable of channel estimation.
...
Exploiting the two-timescale channel property in the RIS-assisted system, the authors of \cite{Huchen} propose a two-timescale channel estimation algorithm that reduces the pilot overhead in the time-averaged sense.
...
Unfortunately, classical channel estimation and beamforming procedures result in an unavoidable drawback in pilot overhead, especially when extremely large-scale RISs are employed. This is because all the above methods adopt separated channel estimation and beamforming, thus forcing the channel estimation step to obtain the full channel matrix, which is at least of size $\mathcal{O}(N)$. 
...
To circumvent the problem of high pilot overhead required for channel estimation in RIS-assisted system, we propose an \ac{IRF}-based approach, where the \ac{IRF} on RIS is utilized for channel estimation\footnote{Simulation codes will be provided to reproduce the results in this paper: \url{http://oa.ee.tsinghua.edu.cn/dailinglong/publications/publications.html}.}.
Specifically, the contributions of this paper can be summarized as follows.
    \begin{itemize}
        ...
        Inspired by optical interference, phase information can be obtained from the interference fringes, while for beamforming only phase information is necessary. Thus, we propose a novel framework that signals are transmitted from the \ac{BS} and the user to RIS simultaneously during CSI acquisition.
        Under this framework, we reveal the phenomenon that electromagnetic interference will occur on RIS, which can be achieved by rotational signaling. which we name as the \ac{IRF}.
        \item
        To exploit this \ac{IRF} for phase estimation, we employ sensing RIS which integrates power detectors to capture the features of \ac{IRF}.
        Each of the low-cost power sensor can know its own phase information independently from IRF.
        We then develop LS, ML and VM-EM phase estimation algorithms that combine the procedures of channel estimation and beamforming, where the required pilot overhead is independent of the number of RIS elements.
        \item
        We derive the \ac{CRLB} of the phase estimation problem based on \ac{IRF}, and its approximated asymptotic expression for channel estimation in the sensing RIS-assisted system.
        Our numerical results demonstrate that this approximated \ac{CRLB} is asymptotically precise, and that our developed algorithms are close to \ac{CRLB}. Our simulation results also demonstrate that. 
        \item
        (delete this item) Simulation results demonstrate that, our proposed signal transmission framework and the corresponding algorithms can obtain precise phase information 
        realize dimension-independent CSI acquisition for beamforming, which achieve the near-optimal capacity, which is the first one to achieve dimension-independent CSI acquisition. 
...
\subsection{Properties of the CRLB}
    ...
    \begin{theorem}[CRLB Approximation by Asymptotic Expansion is Asymptotically Optimal] \label{thm_error_analysis}
        The relative error $r$ of \eqref{eqn:definition g function}, as an approximation of \eqref{eqn:precise_CRLB}, decreases at a rate that is inverse proportional to the interferential SNR $\gamma$, i.e.,
        \begin{equation}
    ...
    \end{IEEEproof}
    \begin{remark}
        From {\bf Theorem \ref{thm_error_analysis}} we can also conclude that the relative error of \eqref{eqn:asymptotic CRLB} is also of order $\mathcal{O}(1/\bar{\gamma})$. 
    \end{remark}
