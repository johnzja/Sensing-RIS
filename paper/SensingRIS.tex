...
\subsection{Achievable Spectral Efficiency under IRF} \label{Achievable Spectral Efficiency under IRF}
    ...
    \red{
    Moreover, for the IRF methods, the BS precoder $\bm w$ is chosen to be the right singular vector that corresponds to the most significant singular value of the BS-RIS channel $\bm G$, which automatically maximizes the total energy received by the RIS from the BS, i.e., 
    \begin{equation}
        {\bm w} = \underset{\left\|{\bm w}\right\|=1}{\rm argmax}\left\|{\bm G}{\bm w} \right\|^2,
    \end{equation}
    and the user's scaler is set to $w'=1$ for all the CSI acquisition schemes. 
    For the LMMSE algorithms, the BS precoder and the RIS phase-shift matrix is updated alternately according to 
    \begin{equation}
        \left\{\begin{aligned}
        ...
        \end{aligned}\right.
    \end{equation}
    where ${\rm norm}({\bm z})$ denotes ${\bm z}/\left\|{\bm z}\right\|$.
    }
