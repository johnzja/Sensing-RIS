...
Moreover, we develop three algorithms for parameter estimation in this system, and we analyzed the achievable asymptotic performance of the proposed vM-EM algorithm through the fixed point perturbation method. 
...
Specifically, the contributions of this paper can be summarized as follows.
    \begin{itemize}
        ...
        Our numerical results verify that our developed vM-EM algorithm numerically achieves the theoretical error decay rate, and that it is close to the \ac{CRLB}.  
        Our simulation results further demonstrate that the proposed signal transmission method and the corresponding algorithms can realize dimension-independent CSI acquisition for beamforming, and can achieve near-optimal spectral efficiency. }
    \end{itemize}
...
\red{
\subsection{Channel Assumptions}\label{Sec2-Subsec1}

We assume a block-fading Rayleigh channel with $B$ symbols in each block, i.e., the i.i.d. Rayleigh random channel matrices $\bm G$ and $\bm f$ are generated once for every consecutive $B$ symbols. Within these $B$ symbols, there are $N_p$ pilot symbols (the number of time slots for pilot signals) for channel estimation, and $N_d$ data symbols for data transmission, where $B=N_p+N_d$. The pilot symbols have a total energy of $E_p$, and the data symbols have a total energy of $E_d$.
Suppose we want to ensure that the average transmit power of each block is $P$, then the energy values $E_p$ and $E_d$ should satisfy 
\begin{equation}
    P=\frac{E_p+E_d}{B}.
\end{equation}
Thus, if the number of pilot symbols $N_p$ are smaller for some CSI acquisition scheme, then the average pilot signal-to-noise ratio (SNR) $\gamma_p = E_p/(N_p \sigma_n^2)$ would be automatically higher, ensuring fair comparison in the sense that equal energy is injected into the channel for different channel estimation schemes.   

The actual transmit power values during CSI acquisition and data transmission are determined by the following equations:  
\begin{equation}
    \begin{aligned}
...
\end{equation}
where $P_{\rm max}$ is the average BS transmit power budget, $P'_{\rm max}$ is the average user transmit power budget, and $E_p, E_d$ jointly determine the power allocation between pilot symbols and data symbols.  
}
...
\subsection{Simultaneous Rotational Signaling and Interference Detection} \label{Simultaneous Rotational Signaling and Interference Detection}
    ...
    \red{The underlying reason for this assumption is that, in contrast to the fast time-varying RIS-user link, the BS-RIS link often exhibits a quasi-static property, thus two-timescale methods in~\cite{Huchen} can be applied to solely estimate the BS-RIS channel.
...
Since the direct link does not affect the creation of the IRF, the RIS phase-shift matrix $\bm \Theta$ can be obtained as if there were no such direct link, up to an undetermined global phase $e^{\ri \phi}$. Note that this IRF-based procedure for obtaining ${\bf\Theta}$ consumes only 3 time slots. After this procedure, two additional uplink training symbols can be transmitted from the user. The uplink channel model is 
\begin{equation}
    \begin{aligned}
...
\subsection{Spectral Efficiency with IRF-based CSI Acquisition} \label{Achievable Spectral Efficiency under IRF}
    ...
    \begin{equation}
        ...
        \end{aligned}\right. \label{eqn:iterative_beamforming}
    \end{equation}
    ...
    We compare our proposed \ac{IRF} channel estimation and beamforming method with classical MMSE-$\bm f$, LMMSE-$\bm H$, and other idealized settings in Fig.~\ref{fig:rate}. 
    ...
    The ``Oracle'' (black dashed line) assumes perfectly-known CSI with iterative beamforming~\eqref{eqn:iterative_beamforming}. 
    In the LMMSE-$\bm H$ method (pink dashed line), we first utilize~\eqref{eqn:LMMSE} to obtain estimates for the cascaded channel~\cite{kundu2021channel,wei2021channel}, and then perform iterative RIS beamforming~\eqref{eqn:iterative_beamforming} based on the estimated channel.
    The MMSE-$\bm f$ method (blue line) is calculated by~\eqref{eqn:MMSE-f-estimator}. 
