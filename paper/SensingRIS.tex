...
To estimate the phase $\varphi$, we need enough observations from the sensor detection signal. For simplicity and without loss of generality, let us assume $\psi(t)=\omega t$. Assume furthermore that $L$ observations $P[l]=P(t_{l})$ at time 
\begin{equation}
...
\subsubsection{ML method}
    ...
    \end{equation}
    then the p.d.f of $s(t) := P(t)-\zeta$ is 
    \begin{equation}
        f_{s(t)}(x) = \frac{1}{A\sigma_{v}^2}I_{0}\left(\frac{\sqrt{\lambda(t) x}}{A\sigma_v^2/2}\right), x \geq 0
    \end{equation}
