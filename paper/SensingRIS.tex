...
\label{Channel Estimation}
    ...
    \end{equation}
    By approximating the MMSE estimator with the best linear estimator and assuming $\sigma_h^2 = \sigma_f^2\sigma_g^2$, an LMMSE estimator can be obtained as 
    \ifx\onecol\undefined
        \begin{equation}
            \begin{aligned}
            & (\hat{\bm{H}\T})_{\rm MMSE} \\
    ...
    \else
        \begin{equation}
            \hat{\bm{H}\T}_{\rm MMSE} = \sqrt{P'_{\rm max}} {\bm Y}_{\rm BS} {\bm F}_{N, N_p}\H \left( P'_{\rm max}{\bm F}_{N, N_p} {\bm F}_{N, N_p}\H + \frac{\sigma_z^2}{\sigma_h^2}{\bm I}_{N}  \right)^{-1},
            \label{eqn:LMMSE}
        \end{equation}
    \fi
    which provides a feasible solution to channel estimation in this RIS-assisted MISO system. In this paper, we name it the linear minimum mean squared error (LMMSE) channel estimation method. 
