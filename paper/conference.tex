...
\subsection{Simultaneous Rotational Signaling and Interference Detection} \label{Simultaneous Rotational Signaling and Interference Detection}
    ...
    Interference allows noncoherent power sensors to perform coherent detection. To clarify this idea, we take only one RIS element into consideration. 
    We find that, if the BS transmit a fixed symbol $s=1$ while the user transmits a rotational symbol $s'=e^{{\rm j}\psi(t)}$, then the detected IRF power will vary over time. 
    Thus, the desired CSI can be drawn from the varying IRF power signal received by sensing RIS, as is shown in Fig.~\ref{fig:protocol}. 
    ...
    This is because, in contrast to the fast time-varying RIS-user link, the BS-RIS link often exhibits a quasi-static property, which provides an opportunity to simplify the phase estimation \cite{Huchen}.
    In practice and our simulations, the phase $\arg({\bm g}_n^{T}{\bm w})$ is calculated based on the geometry, i.e., the locations of the BS and the RIS \cite{bai2014coverage}. 
...
\subsection{IRF Channel Estimation and Beamforming} \label{IRF Channel Estimation and Beamforming}
    ...
    \begin{algorithm}[H] 
        ...
        \end{algorithmic}
    \end{algorithm}
    Note that in each iteration $n$ of {\bf Algorithm~\ref{alg:IRF-Beamforming}}, the data $P(t)$ is independent of other iterations. Thus, a parallel computing scheme results in an $\mathcal{O}(1)$ time complexity. 
