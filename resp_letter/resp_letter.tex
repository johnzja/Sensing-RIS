...
{\color{blue}{\textbf{Authors: } 
Many thanks to the reviewers for their comments on improving the rigor of this paper. In this paper, we assume that the data transmission is preceded by two link establishment steps:
\begin{enumerate}
...
\end{enumerate}
Further, we assume a block-fading channel with blocklength $B$. Within the same block, the random BS-RIS channel matrix $\bm G$ and the RIS-user channel vector ${\bm f}^*$ are realized for only once. After setting $\bm w$ and $\bm\Theta$, the user's downlink SNR 
\begin{equation}
...
\begin{equation}
    C_{\rm SI} = \frac{N_d}{B} \mathbb{E}\left[\log\left( 1+\gamma_{\rm user} \right)\right], \label{eqn:capacity_def}
\end{equation} 
where perfect side information [R1] (channel knowledge) is assumed after the establishment of the RIS-aided reflective link. In our simulations, the expectation in~\eqref{eqn:capacity_def} is numerically approximated by applying the Monte Carlo method. 
