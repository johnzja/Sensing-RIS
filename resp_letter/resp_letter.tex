...
\quad In the revised paper, this issue is addressed as 
}

\begin{framed}
    {\bf Section III-B}\\
    \red{The underlying reason for this assumption is that, in contrast to the fast time-varying RIS-user link, the BS-RIS link often exhibits a quasi-static property, thus two-timescale methods in~[11] can be applied, in which the quasi-static BS-RIS channel $\bm G$ is estimated in a longer timescale, while the frequently-varying RIS-user channel $\bm f$ is estimated in a shorter timescale.
...
In the revised paper, we have integrated the above discussion into {\bf Section IV-D}.  
...
\begin{equation}
    P=\frac{E_p+E_d}{B}.
\end{equation}
\quad Thus, if the number of pilot symbols $N_p$ are smaller for some scheme, then the average pilot signal-to-noise ratio (SNR) $\gamma_p = E_p/(N_p \sigma_n^2)$ would be automatically higher, ensuring fair comparison in the sense that equal energy is injected into the channel for different channel estimation schemes.   

\quad According to the reviewer's advice, we have improved our simulations by replacing the overall transmit SNR $\gamma = P_{\rm BS}/\sigma_n^2$ by the normalized pilot SNR $\gamma_p=E_p/(N_p \sigma_n^2)$ and the normalized data SNR $\gamma_d=E_d/(N_d\sigma_n^2)$, where $B=N_p+N_d$. 
...
%% 
\textbf{Reviewer 1:}
It is understood that the conventional channel estimation technique of {\bf Section II-B} requires $P$ time slots. How is $P$ determined? If $P$ equals to the time slots/pilot cost of the proposed scheme, how would the performance comparison be?

\blue{{
{\bf Authors:} Thanks to the reviewer for pointing out the vague explanation about the number of required pilot slots $P$ in Sec.II.B. 
...
\blue{
{\bf Authors:} Thank you for pointing out these three important issues. The detailed response to these concerns are listed as follows. 
...
\quad In the revised paper, we have clarified the initialization and updating methods of the BS beamformer ${\bm w}$ and the user's scaler $w'$ for our proposed algorithms as well as the baselines, which is shown as follows (some equations are repeated for the sake of clarity). 
}
...
\red{
    A near-optimal solution to this joint active-passive beamforming problem can be solved by the following alternating optimization (AO) loop~[3]:
    \begin{equation}
        \left\{\begin{aligned}
    ...
    However, a more often assumption in real-world RIS-aided systems is that the locations of the BS and the RIS are usually fixed, implying that the BS-RIS channel $\bm G$ enjoys a much longer channel coherence time compared with the RIS-user channel $\bm f$~[11]. Thus, in order to mitigate the computational cost, a suboptimal solution can be immediately obtained by designing $\bm w$ and $\bm\Theta$ in a one-shot manner, i.e., 
    \begin{equation}
        \begin{aligned}
    ...
    \end{equation}
    where $\bm G = \left[\bm g_{1}, \cdots, \bm g_{N}\right]\T$. Note that this method is equivalent to optimizing the upper bound of the target function:
    \begin{equation}
...
{\bf Section IV-C}
    ...
    Except for CS-based methods [5], all the channel estimation methods [11], [13], [24] require the pilot overhead to be linearly dependent on $N$, while our IRF method needs a constant number of exactly 3 pilot slots per user, regardless of the number of RIS elements. 
}
\end{framed}
...
\quad In the revised paper, we have addressed this issue in the simulation result part as:
}}
...
\blue{\textbf{Authors: } 
Many thanks for the reviewer's positive attitude on the novelty and correctness of this paper. Also, as is stressed by the reviewer, the previous version of this paper lacks a series of sound information-theoretic analyses. We agree with the reviewer that the analytical results are necessary for the high-quality journals like IEEE T-IT. To this end, we have tried our best to provide achievable bounds for our proposed interference random field (IRF)-based CSI acquisition schemes. These newly-added theorems and lemmas are mainly related to detection and estimation theory, which may match the broader scope of this journal. For you to check, we have provided the theoretical contents in the box below with some slight modifications, e.g., the number of equations and lemmata may be different from those in the revised paper. 
...
\red{
    ...
    \begin{align}\label{ineq:gradient_bound}
...
\blue{
\quad The newly-added proofs of the above lemmas and theorems are provided in the revised paper. }
